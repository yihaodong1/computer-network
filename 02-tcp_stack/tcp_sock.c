...
{
	tsk->ref_cnt -= 1;
	log(DEBUG, IP_FMT"-->"IP_FMT" ref_cnt: %d",HOST_IP_FMT_STR(tsk->sk_sip), \
		HOST_IP_FMT_STR(tsk->sk_dip), tsk->ref_cnt);
	if(tsk->ref_cnt == 0){
		log(DEBUG, "free "IP_FMT":%u -->"IP_FMT":%u", \
			HOST_IP_FMT_STR(tsk->sk_sip), tsk->sk_sport, \
...
struct tcp_sock *tcp_sock_lookup_established(u32 saddr, u32 daddr, u16 sport, u16 dport)
{
	...
	int hash = tcp_hash_function(saddr, daddr, \
				sport, dport); 
...
int tcp_sock_connect(struct tcp_sock *tsk, struct sock_addr *skaddr)
{
	...
	tsk->sk_sip = 0xa000002;
	tsk->sk_sport = 12346;
	tcp_bind_hash(tsk);
	tcp_send_control_packet(tsk, TCP_SYN);
	tcp_set_state(tsk, TCP_SYN_SENT);
	tcp_hash(tsk);
	assert(sleep_on(tsk->wait_connect) >= 0);
	log(DEBUG, IP_FMT"-->"IP_FMT" connect",HOST_IP_FMT_STR(tsk->sk_sip), HOST_IP_FMT_STR(tsk->sk_dip));
...
void tcp_sock_close(struct tcp_sock *tsk)
{
	...
	if(tsk->state == TCP_ESTABLISHED || tsk->state == TCP_SYN_RECV){
